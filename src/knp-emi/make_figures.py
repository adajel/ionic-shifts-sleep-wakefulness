...
def plot_3D_concentration(fname_in, fname_out, dt, Tstop, x, tag, \
        save_frequency, f_input, f_decay):
    ...
    i_input = f_input * np.full(len(phi_M), 1.0)
    i_decay = f_decay * (np.array(K_e) - K_e_init)
    ...
    f_i_input = open(f'results/{fname_in}/i_input.txt', "w")
    for p in i_input:
        f_i_input.write("%.10f \n" % p)
    f_i_input.close()

    f_i_decay = open(f'results/{fname_in}/i_decay.txt', "w")
    for p in i_decay:
        f_i_decay.write("%.10f \n" % p)
    f_i_decay.close()

    f_tr_K_e = open(f'results/{fname_in}/tr_K_e.txt', "w")
    for p in tr_K_e:
        f_tr_K_e.write("%.10f \n" % p)
    f_tr_K_e.close()

    f_tr_K_i = open(f'results/{fname_in}/tr_K_i.txt', "w")
    for p in tr_K_i:
        f_tr_K_i.write("%.10f \n" % p)
    f_tr_K_i.close()

    f_tr_Na_e = open(f'results/{fname_in}/tr_Na_e.txt', "w")
    for p in tr_Na_e:
        f_tr_Na_e.write("%.10f \n" % p)
    f_tr_Na_e.close()

    f_tr_Na_i = open(f'results/{fname_in}/tr_Na_i.txt', "w")
    for p in tr_Na_i:
        f_tr_Na_i.write("%.10f \n" % p)
    f_tr_Na_i.close()


    f_tr_Cl_e = open(f'results/{fname_in}/tr_Cl_e.txt', "w")
    for p in tr_Cl_e:
        f_tr_Cl_e.write("%.10f \n" % p)
    f_tr_Cl_e.close()

    f_tr_Cl_i = open(f'results/{fname_in}/tr_Cl_i.txt', "w")
    for p in tr_Cl_i:
        f_tr_Cl_i.write("%.10f \n" % p)
    f_tr_Cl_i.close()
...
if __name__ == "__main__":
    ...
    f_input = config["f_input"]
    f_decay = config["f_decay"]
