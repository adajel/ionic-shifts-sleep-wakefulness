...
fname_out = "dt1"
dt = 1.0               # ms
Tstop = 1000*0.5       # ms short simulation
...
time = np.arange(0, Tstop/(1000), dt/(1000))          # s

# NB! just for plotting
k_dec = 2.0e-7 * F
j_decay = - k_dec*(K_e - K_e_init)
i_stim = [g_syn_bar * (1.0e-3  < t) * (t < 80*1000) for t in time]
...
# save figure to file
plt.savefig('ode_dynamics_short.png', format='png')
plt.close()

phi_M = potential_history_g[:, 2]
K_e = K_e_history[:, 2]
K_g = K_g_history[:, 2]
Na_e = Na_e_history[:, 2]
Na_g = Na_g_history[:, 2]
...
f_phi_M = open(f'results/phi_M_{fname_out}.txt', "w")
for p in phi_M:
    f_phi_M.write("%.10f \n" % p)
f_phi_M.close()

f_K_e = open(f'results/K_ECS_{fname_out}.txt', "w")
for p in K_e:
    f_K_e.write("%.10f \n" % p)
f_K_e.close()

f_K_g = open(f'results/K_ICS_{fname_out}.txt', "w")
for p in K_g:
    f_K_g.write("%.10f \n" % p)
f_K_g.close()

f_Na_e = open(f'results/Na_ECS_{fname_out}.txt', "w")
for p in Na_e:
    f_Na_e.write("%.10f \n" % p)
f_Na_e.close()

f_Na_g = open(f'results/Na_ICS_{fname_out}.txt', "w")
for p in Na_g:
    f_Na_g.write("%.10f \n" % p)
f_Na_g.close()

f_Cl_e = open(f'results/Cl_ECS_{fname_out}.txt', "w")
for p in Cl_e:
    f_Cl_e.write("%.10f \n" % p)
f_Cl_e.close()

f_Cl_g = open(f'results/Cl_ICS_{fname_out}.txt', "w")
for p in Cl_g:
    f_Cl_g.write("%.10f \n" % p)
f_Cl_g.close()
